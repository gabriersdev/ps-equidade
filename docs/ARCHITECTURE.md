...
## Navegação e Estado (Server Components)
- **Estados via URL (Search Params)**: A aplicação prioriza manter componentes como *Server Components* sempre que possível. Estados de visualização, como a ordenação da lista de aulas, são gerenciados utilizando os `searchParams` da URL (ex: `?sort=date-asc`) em vez de estados locais (`useState`).
